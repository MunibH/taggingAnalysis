...
for isess = 1:numel(un.sessobj)
    ...
    for iprobe = 1:numel(un.sessobj(isess).psth)
        ...
        allregion = cat(1,allregion, cellstr(repmat(un.meta(isess).region{iprobe},numel(un.sesspar(isess).quality{iprobe}),1)));
    end
...
region.alm = contains(allregion,'ALM');
region.m1 = contains(allregion,'tjM1');
...
numBootstraps = 1000;
plotTemplates = 1;
[rez.nnmflabels.alm, ~, rez.H_final.alm] = doNNMF(activityRL(:,region.alm)', clusterRange, numBootstraps, plotTemplates);
[rez.nnmflabels.m1, ~, rez.H_final.m1] = doNNMF(activityRL(:,region.m1)', clusterRange, numBootstraps, plotTemplates);
...
for r = 1:numel(regions)
    ...
    rpsth = psth(:,region.(rr),:);

    clusterLabels = unique(rez.nnmflabels.(rr));
    nClusters = numel(clusterLabels);
    nUnitsPerCluster = accumarray(rez.nnmflabels.(rr), 1);
    ...
    for j = 1:nClusters
        ...
        clumask = rez.nnmflabels.(rr)==clusterLabels(j);
        temp = squeeze(nanmean(rpsth(:,clumask,:),2));
    ...
    [s,sortnmf] = sort(rez.nnmflabels.(rr));
    sel = (rpsth(:,:,1) - rpsth(:,:,2));
    ...
    title(['Selectivity, ' rr],'fontsize',11,'fontweight','normal')
    ...
    hold on
    for j = 1:nClusters
        n = sum(rez.nnmflabels.(rr)==clusterLabels(j));
        ntag = sum(rez.nnmflabels.(rr)==clusterLabels(j) & ismember(allqual(region.(rr)),'tagged'));
        prop(j) = ntag/n * 100;
    end
