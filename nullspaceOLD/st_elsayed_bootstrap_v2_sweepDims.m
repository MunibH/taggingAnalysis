...
% nNPdims = [4,6,10,13];% nNPDims as last arg if you want to change nDims per subspace [4,6,10,13]
nNPdims = [4,6,10];% nNPDims as last arg if you want to change nDims per subspace [4,6,10,13]

disp('finding null and potent spaces')
for nDims = nNPdims
    clearvars -except obj meta params me sav datapth kin rt nDims nNPdims
    for sessix = 1:numel(meta)
        ...
        rez(sessix) = singleTrial_elsayed_np(trialdat_zscored, obj(sessix), me(sessix), ...
            params(sessix), cond2use, cond2proj, nullalltime, onlyAW, delayOnly, responseOnly, nDims); % nNPDims as last arg if you want to change nDims per subspace
    ...
    plotVarianceExplained % plot variance explained plots (for sweeping dims)
    % plotVarianceExplained_main % for main number of dims
    % plotEpochVarianceExplained % for sweeping dims, change filename to save for sweep dims
    % alignmentPlots % plot Alignment plots for NP, Choice, Ramp
end
disp('DONE')
