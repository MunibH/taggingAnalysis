...
class KeypointsNetwork(nn.Module):
    ...
    def train_model(
        ...
        running_train_loss = np.zeros((n_iter,1))
        running_learning_rate = np.zeros((n_iter,1))
        running_ve = np.zeros((n_iter,1))
        ...
        for epoch in tqdm(
            ...
            running_train_loss[epoch] = train_loss
            running_learning_rate[epoch] = optimizer.param_groups[0]["lr"]
            ...
            if epoch % 20 == 0 or epoch == n_iter - 1:
                ...
                running_ve[epoch] = ve_all
                pstr += f"time {time.time()-tic:.1f}s"
                if verbose:
                    print(pstr)

        if not_list:
            return y_pred_all[0], ve_all[0], spks_pred_all[0], ve_neurons[0], itest[0], running_train_loss , running_learning_rate ,running_ve 
        else:
            return y_pred_all, ve_all, spks_pred_all, ve_neurons, itest, running_train_loss , running_learning_rate ,running_ve 
