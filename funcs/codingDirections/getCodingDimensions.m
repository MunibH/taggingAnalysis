function rez = getCodingDimensions(rez,obj,params)


cond2use = rez.cond2use;

cd_labels = rez.cd_labels;
cd_epochs = rez.cd_epochs;
cd_times = rez.cd_times;

if isfield(rez,'rampcond')
    rampcond = rez.rampcond;
    ramp_epochs = rez.ramp_epochs;
    ramp_times = rez.ramp_times;
end
...
%     rez(sessix).psth = standardizePSTH(obj(sessix));
    ...
    % ------------------------------------------
    temp = permute(rez(sessix).psth,[1 3 2]); % (time,cond,neurons), permuting to use tensorprod() on next line for the projection
    rez(sessix).cd_proj = tensorprod(temp,rez(sessix).cd_mode,3,1); % (time,cond,cd), cond is in same order as con2use variable defined at the top of this function
    
    temp = permute(obj(sessix).trialdat,[1 3 2]); % (time,trials,neurons)
    rez(sessix).cd_trialdat = tensorprod(temp,rez(sessix).cd_mode,3,1);
