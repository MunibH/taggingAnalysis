...
% params.condition(end+1)     = {'hit&Nlicks==2&trialTypes==1'};
% params.condition(end+1)     = {'hit&Nlicks==2&trialTypes==2'};
% params.condition(end+1)     = {'hit&Nlicks==2&trialTypes==3'};
% params.condition(end+1)     = {'hit&Nlicks==5&trialTypes==1'};
% params.condition(end+1)     = {'hit&Nlicks==5&trialTypes==2'};
% params.condition(end+1)     = {'hit&Nlicks==5&trialTypes==3'};
...
% cond2use = [4,5,6]; % R2 (right,center,left)
% cond2use = [7,8,9]; % R5 (right,center,left)

trials = cell2mat(params.trialid(cond2use)');
yl = nan;
for i = 1:numel(cond2use)-1
    yl(i) = numel(params.trialid{cond2use(i)});
end
yl = cumsum(yl);
...
trials = params.trialid(cond2use);
...
f = figure;
f.Position = [506         372        1133         477];
ax = prettifyAxis(gca);
hold on;
for i = 1:numel(trials)
    toplot = kin.dat(:,trials{i},featix);
    plot(obj.time,rad2deg(toplot),'Color',cols{i})
end
plotEventTimes(ax,params.eventTimes,'w')
xlim([params.tmin,params.tmax])
...
% find median time to first lick contact
licktimes = obj.bp.ev.lickL;
gc = obj.bp.ev.goCue;

firstLickTime = cellfun(@(lt, t) lt(find(lt >= t, 1)), licktimes, num2cell(gc), 'uni',0);

% Identify empty entries
isEmpty = cellfun(@isempty, firstLickTime);

% Replace empty entries with NaN
firstLickTime(isEmpty) = {NaN};

firstLickTime = cell2mat(firstLickTime')';

medRT = nanmedian(firstLickTime-gc);

edges = [0 medRT];
tix = findTimeIX(obj.time,edges,1);
% tix = 1:numel(obj.time);

X = permute(obj.trialdat(tix,:,:),[1,3,2]);
X = reshape(X,size(X,1)*size(X,2),size(X,3)); % (time*trials,neurons)

feat = 'tongue_angle';
% feat = 'jaw_phase';
featix = find(ismember(kin.featLeg,feat));

y_ = fillmissing(kin.dat(tix,:,featix),'constant',0);
y = reshape(y_,size(y_,1)*size(y_,2),size(y_,3));

W = inv(X'*X)*(X'*y);
yhat = X*W;

% Compute the total variance of y
total_variance_y = var(y, [], 1); % variance along the time dimension (1st dimension)

% Compute the residuals
residuals = y - yhat;

% Compute the variance of the residuals
variance_residuals = var(residuals, [], 1);

% Compute the R^2 for each feature
R2 = 1 - (variance_residuals ./ total_variance_y);

% Display the R^2 values
disp('R^2 values for each feature:');
disp(R2);

f = figure;
ax = subplot(1,2,1);
imagesc(reshape(y,size(y_,1),size(y_,2),size(y_,3))')
ax = subplot(1,2,2);
imagesc(reshape(yhat,size(y_,1),size(y_,2),size(y_,3))')
...
proj.cd = tensorprod(trialdat_zscored,W,3,1);
...
recon.cd = tensorprod(proj.cd,W,3,2);

% project null and potent recons onto cd, and reconstruct
proj.nullcd = tensorprod(recon.null,W,3,1);
proj.potentcd = tensorprod(recon.potent,W,3,1);

recon.nullcd = tensorprod(proj.nullcd,W,3,2);
recon.potentcd = tensorprod(proj.potentcd,W,3,2);


%% plot CD projections

cond2plot = [4,5,6]; % r,c,l
